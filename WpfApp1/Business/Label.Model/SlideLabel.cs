...
namespace YellowstonePathology.Business.Label.Model
{
    public class SlideLabel
    {
        ...
        public static string GetZplCommands(string orderedOnId, DateTime pBirthdate, string patientName, string specimen)
        {
            StringBuilder commands = new StringBuilder();
            commands.Append("^XA");
            commands.Append("^FT20,120^BY2^A0N,50,30 ^BC,100,N,N,N,A^FD" + orderedOnId + "^FS");
            commands.Append("^FO20,130^ADN,18,10^FD" + orderedOnId + ": " + pBirthdate.ToString("MM/dd/yyyy") + "^FS");
        ...
        public void DrawLabel(int x, int y, System.Drawing.Printing.PrintPageEventArgs e)
        {
			...
			string barcodeId = Business.BarcodeScanning.BarcodePrefixEnum.HBLK + this.m_AliquotOrder.AliquotOrderId;
