...
{
    public class RiverstoneOBXView
    {
        ...
        public RiverstoneOBXView(YellowstonePathology.Business.Test.AccessionOrder accessionOrder, string reportNo, int obxCount)
        {
            this.m_NteCount = 1;
        ...
        public virtual void ToXml(XElement document, string observationResultStatus)
        {
        ...
        protected void AddNextObxWithAttributeElement(string attribute, string value, XElement document, string observationResultStatus)
        {
            string escapedString = value;
            ...
            XElement obx03Element = new XElement("OBX.3");
            YellowstonePathology.Business.Helper.XmlDocumentHelper.AddElement("OBX.3.1", attribute, obx03Element);
            obxElement.Add(obx03Element);
        ...
        protected void HandleLongStringAddNTE(string value, XElement document)
        {
            if (string.IsNullOrEmpty(value) == false)
            {
                string[] textSplit = value.Split(new string[] { "\n", "\r\n", "\r" }, StringSplitOptions.RemoveEmptyEntries);
                ...
                {                    
                    ...
                    foreach (string line in lines)
                    {
                        this.AddNextNteElement(line, document);
                    }
                }
            }
        }
